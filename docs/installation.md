...
1. **[uv](https://docs.astral.sh/uv/)** — Python 3.14+ toolchain and runner.
...
3. **A reachable self-hosted Ollama** with the models pulled:
   ...
   `--ollama-host` or the `OLLAMA_HOST` env var (see [configuration](./configuration.md)).

## Install

From source (until a released version is on PyPI — see [releasing](./releasing.md)):
...
required model pulled. Next: the [quickstart](./quickstart.md).
